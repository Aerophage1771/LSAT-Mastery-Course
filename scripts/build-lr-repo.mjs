 ...
 * from each *Drill*.tsx file in LR content modules 1-20, 55, 59, and 60. One lesson per LR route module.
 */
import fs from 'fs';
...
function buildLessonFile(moduleNum, moduleTitle, drills) {
  const lessonNum = getRouteLessonNumber(moduleNum);
  ...
  const routeModuleLabel = `Module ${lessonNum}: ${moduleTitle}`;
...
export const ${componentName}: Lesson = {
  id: "${id}",
  title: "${routeModuleLabel}",
  content: [
    { type: 'h2', text: '${routeModuleLabel.replace(/'/g, "\\'")}' },
...
function getRouteLessonNumber(moduleNum) {
  if (moduleNum <= 9) return moduleNum;
  if (moduleNum === 60) return 10;
  if (moduleNum >= 10 && moduleNum <= 20) return moduleNum + 1;
  if (moduleNum === 55) return 22;
  if (moduleNum === 59) return 23;
  return moduleNum;
}
...
function main() {
  ...
  for (const moduleNum of lrModules) {
    ...
    if (!fs.existsSync(moduleDir)) {
      ...
      const routeLessonNumber = getRouteLessonNumber(moduleNum);
      const fileName = `Lesson${routeLessonNumber}_Module${moduleNum}_Questions.tsx`;
      fs.writeFileSync(path.join(MODULE48_DIR, fileName), content, 'utf8');
      console.log(`Wrote ${fileName} (no drills)`);
      continue;
    }

    const files = fs.readdirSync(moduleDir).filter((f) => {
      if (!f.endsWith('.tsx')) return false;
      if (moduleNum === 60) return /^Lesson\d+.*\.tsx$/.test(f);
      return f.includes('Drill');
    });
