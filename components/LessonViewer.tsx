...
export const LessonViewer: React.FC<LessonViewerProps> = ({
    ...
    return lines.map((line, index) => {
      ...
      if (line.startsWith('> '))
        return (
          <div
            key={index}
            className="my-5 px-5 py-4 bg-slate-50 rounded-lg border border-slate-200/80 text-[15px] text-slate-600 leading-relaxed"
            dangerouslySetInnerHTML={{ __html: parseInlineStyles(line.replace('> ', '')) }}
          />
        );
      ...
      switch (block.type) {
        ...
        case 'callout': {
          ...
          return (
            <div
              key={index}
              className={`my-6 pl-4 pr-5 py-4 rounded-lg border border-slate-200/60 border-l-[3px] ${accentColor} ${bgColor}`}
            >
              {block.title && (
  ...
  return (
    <div className={containerClasses}>
      ...
      <div className="max-w-none">{typeof content === 'string' ? renderMarkdown(content) : renderBlocks(content)}</div>
    </div>
  );
};
